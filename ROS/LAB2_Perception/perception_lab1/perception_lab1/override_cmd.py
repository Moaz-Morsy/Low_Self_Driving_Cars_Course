...
class my_node (Node):
    def __init__(self):
        ...
        self.emergency_bool = False
        self.emergency_min_dist = 0

        self.create_subscription(LaserScan,"/scan",self.scan_cb, self.qos_profile_s)
        self.create_subscription(CameraEmergency,"/cameraEmergency",self.camera_cb, self.qos_profile_s)
    ...
    def camera_cb(self,message):
        self.emergency_bool = message.close_obstacle_detected
        self.emergency_min_dist = message.min_distance
        # self.get_logger().info(f"emergency bool is {self.emergency_bool}")
        # self.get_logger().info(f"emergency min dist is {self.emergency_min_dist}")
    ...
    def timer_call(self):
        msg = Twist()
        # if self.laser_data_45 == [] or self.laser_data_315 == [] or not self.emergency_bool:
            # self.get_logger().info(f"Wait for lidar or camera to start")
        try:
            if min(self.laser_data_45)<0.5 or min(self.laser_data_315)<0.5 or self.emergency_bool:
