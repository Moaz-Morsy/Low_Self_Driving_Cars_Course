...
class my_node (Node):
    def __init__(self):
        super().__init__("track_3D_node")
        self.get_logger().info("3D tracking subscriber is started")

        MODEL_PATH = "/home/moaz/Desktop/test_tracking/CenterTrack/models/nuScenes_3Dtracking.pth"
        TASK = "tracking,ddd"
        DATASET = "nuscenes"
        THRESHOLD = 0.1
        self.opt = opts().init("{} --load_model {} --dataset {} --track_thresh {} --debug 2 --test_focal_length 450".format(TASK, MODEL_PATH,DATASET,THRESHOLD).split(' '))
    ...
    def img_cb(self,message):
        ...
        for dict in ret:
            print(dict['tracking_id'], dict['score'],dict['dep']/2)
